...
MoETrafficManagerAccelSim::MoETrafficManagerAccelSim(const Configuration &config,
...
MoETrafficManagerAccelSim::~MoETrafficManagerAccelSim()
{
}

// Node range for a named source/destination in the traffic matrix.
struct AccelNodeRange {
...
static vector<string> _AccelSplitTabs(const string &line)
{
...
void MoETrafficManagerAccelSim::_LoadTrafficMatrix(const Configuration &config)
{
  ...
  if (matrix_file != "") {
    ...
    for (size_t r = 0; r < lines.size(); ++r) {
      ...
      for (size_t t = 1; t < tokens.size(); ++t) {
        if (_AccelIsValidName(tokens[t])) { found = true; break; }
      }
      if (found) {
        header_row = (int)r;
        for (size_t t = 1; t < tokens.size(); ++t)
    ...
    if (header_row < 0) {
      cerr << "Error: Could not find header row with node/router names." << endl;
      exit(-1);
    }

    cout << "MoEAccelSim: Header at line " << header_row
         << ", " << col_names.size() << " dest columns: ";
    for (size_t i = 0; i < col_names.size(); ++i) cout << col_names[i] << " ";
    cout << endl;

    // Resolve column names
    vector<AccelNodeRange> col_ranges;
    ...
    // Parse data rows
    cout << "MoEAccelSim: Distributing to nodes (bytes_per_packet=" << bytes_per_packet << "):" << endl;
    for (size_t r = header_row + 1; r < lines.size(); ++r) {
      ...
      if (row_name.find("sum") != string::npos || row_name.find("Sum") != string::npos)
        continue;
      if (!_AccelIsValidName(row_name)) {
        cout << "MoEAccelSim:   Skipping row '" << row_name << "'" << endl;
        continue;
      }

      AccelNodeRange src = _AccelParseNodeRange(row_name, num_routers, _nodes);
      if (src.first < 0) {
        cout << "MoEAccelSim:   Skipping row '" << row_name << "' (invalid)" << endl;
        continue;
      }
    ...
    // Default: all-to-all uniform across Xbar and HBM router pairs
    for (int sr = 0; sr < num_routers; ++sr) {
      if (gHBMNetAccelRouterConc[sr] == 0) continue;  // skip MC routers
      for (int dr = 0; dr < num_routers; ++dr) {
        if (sr == dr) continue;
        if (gHBMNetAccelRouterConc[dr] == 0) continue;  // skip MC routers
        int src_conc  = gHBMNetAccelRouterConc[sr];
        int dst_conc  = gHBMNetAccelRouterConc[dr];
        int src_first = gHBMNetAccelRouterFirstNode[sr];
        int dst_first = gHBMNetAccelRouterFirstNode[dr];
  ...
  cout << "MoEAccelSim Traffic Matrix loaded:" << endl;
  ...
  for (int s = 0; s < _nodes && shown < 20; ++s) {
    ...
    if (total > 0) {
      int rtr = hbmnet_accelsim_node_to_router(s);
      cout << "    Node " << s << " (router " << rtr
           << ", port " << hbmnet_accelsim_node_to_port(s) << "): "
...
void MoETrafficManagerAccelSim::_Inject()
{
...
bool MoETrafficManagerAccelSim::_SingleSim()
{
  _moe_total_packets = 0;
  for (int s = 0; s < _nodes; ++s) {
    _moe_dest_rr[s] = 0;
  ...
  cout << "MoEAccelSim: Starting batch of " << _moe_total_packets
  ...
  cout << "======================================" << endl;
  cout << "MoEAccelSim Batch Completion Time = " << completion_time << " cycles" << endl;
  ...
  cout << "=== Per-Router Injection/Ejection Analysis (AccelSim) ===" << endl;
  int num_routers = _net[0]->NumRouters();

  const vector<vector<int>> &dist = gHBMNetAccelDistMissFabric;
  ...
  for (int rtr = 0; rtr < num_routers; ++rtr) {
    ...
    double inject_rate = (completion_time > 0) ? (double)total_inject / (double)completion_time : 0;
    double eject_rate  = (completion_time > 0) ? (double)total_eject  / (double)completion_time : 0;

    // Transit approximation
    long long transit_pkts = 0;
    if (!dist.empty() && (int)dist.size() > rtr) {
      for (int sr = 0; sr < num_routers; ++sr) {
        for (int dr = 0; dr < num_routers; ++dr) {
          if (sr == dr || sr == rtr || dr == rtr) continue;
          if (dist[sr][rtr] >= 0 && dist[rtr][dr] >= 0 &&
              dist[sr][dr] >= 0 &&
              dist[sr][rtr] + dist[rtr][dr] == dist[sr][dr]) {
            int sf = gHBMNetAccelRouterFirstNode[sr], sc = gHBMNetAccelRouterConc[sr];
            int df = gHBMNetAccelRouterFirstNode[dr], dc = gHBMNetAccelRouterConc[dr];
            for (int si = 0; si < sc; ++si) {
              if (sf + si >= _nodes) continue;
              for (int di = 0; di < dc; ++di) {
                if (df + di >= _nodes) continue;
    ...
    double transit_rate = (completion_time > 0) ? (double)transit_pkts / (double)completion_time : 0;

    string rtype;
    if (rtr < 2)
      rtype = "Xbar" + to_string(rtr);
    else if (rtr < 2 + gHBMNetAccelK)
      rtype = "MC" + to_string(rtr - 2);
    else
      rtype = "HBM" + to_string(rtr - 2 - gHBMNetAccelK);

    cout << "  Router " << rtr << " (" << rtype
         << ", conc=" << conc << "):"
